...
2. `<rule ref="Lipe.DB.CalcFoundRows" />` for detecting the deprecated uses of MySQL `SQL_CALC_FOUND_ROWS`.
3. `<rule ref="Lipe.PHP.DisallowNullCoalesceInCondition" />` for detecting using `??` in conditions.
4. `<rule ref="Lipe.PHP.DisallowNullCoalesceInForLoops" />` for detecting using `??` in for loops.
